  ...
  <v-container class="fill-height" fluid>
    <v-responsive class="d-flex justify-center fill-height">
      ...
      <v-card class="rounded-xl" style="height: 84%" elevation="0">
        <v-card-text class="text-center text-h6">Select a window:</v-card-text>
        <v-card-actions>
        ...
        <v-card-actions>
          ...
          <v-text-field label="DLL" rounded variant="solo-filled" readonly @click="readPath" v-model="dll_path"></v-text-field>
        </v-card-actions>
        <v-card-actions>
          <v-btn style="margin: 0 auto 0 auto; margin-top: 20px" variant="elevated" color="primary" class="w-50" @click="inject_dll">
            Inject
          </v-btn>
        </v-card-actions>
        <v-card-actions>
          <v-btn style="margin: 0 auto 0 auto;" variant="elevated" color="primary" class="w-50" @click="update_process_list">
            Refresh
          </v-btn>
        </v-card-actions>
